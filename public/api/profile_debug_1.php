...
try {
    ...
    if ($sellerProfile) {
        ...
        $allListings = $db->prepare("SELECT id, seller_id, status FROM listings WHERE seller_id = ?");
        $allListings->execute([$sellerProfile['id']]);
        $out['step3b_all_listings_for_seller'] = $allListings->fetchAll();
    ...
    echo json_encode(['ok'=>true,'data'=>$out], JSON_PRETTY_PRINT);
} catch (Exception $e) {
